...
bool readBlock( uint32_t blockaddr, uint8_t *buffer ) {  // Adjust the address based on the type of card
  blockaddr <<= blockScaling;
  uint8_t reply;
  uint32_t i, response;
  reply = sendCommand( CMD17, blockaddr, (uint8_t *)&response );  //check response ==0
  if(reply!=0) return false;
  uint8_t b;
  do {
      b=rxByteSSI3();
      if(b!=0xff && b!=0xfe) return false;
  } while(b==0xff);  //read 512 byte block
  for(i=0;i<512;i++) {
      buffer[i]=rxByteSSI3();
  }  // dump 2 bytes crc
  rxByteSSI3();
  rxByteSSI3();  // Return true for a successful read.
  return true;
}
