...
// FOR TIMING DELAYS.
    //writeLCD() will use three of these macros
    //-- assertCS(), assertWR(), and writeDB() writes its argument to all eight DB pins  (assert - true sets the signal low)
    //--instead of directly accessing the GPIO port pins.
    writeDB(c);
    assertCS(true);//CS : Active low chip select signal.
    assertWR(true);///WR: Active low write signal.
    assertWR(true);///WR: Active low write signal.
    assertWR(false);///WR: Active low write signal.
    assertCS(false);
}
...
void initLCD( bool orientation ) {
  ...
  GPIO_PORTM[GPIO_AFSEL] &= ~GPIO_ALLPINS;

  //no open drain
 /* GPIO_PORTQ[GPIO_ODR] &= ~GPIO_PIN_4;
  GPIO_PORTN[GPIO_ODR] &= ~(GPIO_PIN_2 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5);
  GPIO_PORTM[GPIO_ODR] &= ~GPIO_ALLPINS;*/
  ...
  GPIO_PORTM[GPIO_DIR] &= ~GPIO_ALLPINS;
  GPIO_PORTM[GPIO_DEN] |= GPIO_ALLPINS;
